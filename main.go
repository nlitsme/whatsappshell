...
var dbAddress = flag.String("db-address", "file:whatsappshell.db?_foreign_keys=on", "Database address")
...
func usage() {
	...
	log.Errorf("   pair-phone <number>")
	log.Errorf("   reconnect")
	log.Errorf("   logout")
	log.Errorf("   appstate [all | resync] <types...>")
	log.Errorf("   request-appstate-key <ids...>")
	log.Errorf("   unavailable-request <chat JID> <sender JID> <message ID>")
	log.Errorf("   checkuser <phone numbers...>")
	// log.Errorf("   checkupdate")
	log.Errorf("   subscribepresence <jid>")
	log.Errorf("   presence <available/unavailable>")
	log.Errorf("   chatpresence <jid> <composing/paused> [audio]")
	log.Errorf("   privacysettings")
	log.Errorf("   setprivacysetting <setting> <value>")
	log.Errorf("   getuser <jids...>")
	log.Errorf("   mediaconn")
	log.Errorf("   raw  <json-node>")
	log.Errorf("   listnewsletters")
	log.Errorf("   getnewsletter  <jid>")
	log.Errorf("   getnewsletterinvite <channel-url>")
	log.Errorf("   livesubscribenewsletter <jid>")
	log.Errorf("   getnewslettermessages <jid> [count] [before id]")
	log.Errorf("   createnewsletter <name>")
	log.Errorf("   getavatar <jid> [existing ID] [--preview] [--community]")
	log.Errorf("   getgroup <jid>")
	log.Errorf("   addgroup [props] <jid> ...")
	log.Errorf("         props: {name|topic|invite}:\"...\"")
	log.Errorf("   jsonadd <groupdef.json>")
	log.Errorf("   subgroups <jid>")
	log.Errorf("   communityparticipants <jid>")
	...
	log.Errorf("   updateparticipant <jid> <action> <numbers...>")
	log.Errorf("   getrequestparticipant <jid>")
	log.Errorf("   getstatusprivacy")
	log.Errorf("   setdisappeartimer <jid> <days>")
	log.Errorf("   setdefaultdisappeartimer <days>")
	log.Errorf("   send <jid> <text>")
	log.Errorf("   sendpoll <jid> <max answers> <question> -- <option 1> / <option 2> / ...")
	...
	log.Errorf("   setpushname <name>")
	log.Errorf("   setstatus <message>")
	log.Errorf("   archive <jid> <action>")
	log.Errorf("   mute <jid> <action>")
	log.Errorf("   pin <jid> <action>")
	log.Errorf("   getblocklist")
	log.Errorf("   block <jid>")
	log.Errorf("   unblock <jid>")
	log.Errorf("   labelchat <jid> <labelID> <action>")
	log.Errorf("   labelmessage <jid> <labelID> <messageID> <action>")
	log.Errorf("   editlabel <labelID> <name> <color> <action>")
	log.Errorf("   sendbotmsg <inline jid (optional)> <text>")
	log.Errorf("   fetchbotprofiles")
}
...
func handleCmd(cmd string, args []string) {
	...
	case "pair-phone":
		...
		linkingCode, err := cli.PairPhone(context.Background(), args[0], true, whatsmeow.PairClientChrome, "whatsappshell")
		if err != nil {
			panic(err)
		}
    ...
    case "latestversion":
        resp, err := whatsmeow.GetLatestVersion(context.Background(), nil);
		if err != nil {
			log.Errorf("Failed to check for updates: %v", err)
		} else {
			if *resp == store.GetWAVersion() {
                log.Infof("Client is up to date: %v", resp)
			} else if store.GetWAVersion().LessThan(*resp) {
                log.Warnf("Client is outdated: %v < %v", store.GetWAVersion(), resp)
			} else {
                log.Infof("Client is newer than latest: %v > %v", store.GetWAVersion(), resp)
			}
        }
...
func CreateInviteGroup(spec InviteGroupSpec) {
	...
	for _, v := range grp.Participants {
		if v.AddRequest != nil {
			SendInvite(grp, v.JID, v.AddRequest, spec.Invite)
		}
	}
...
func SendInvite(grp *types.GroupInfo, JID types.JID, addreq *types.GroupParticipantAddRequest, invmsg string) {
	msg := &waE2E.Message{
		GroupInviteMessage: &waE2E.GroupInviteMessage{
			GroupJID: proto.String(grp.JID.String()),
			InviteCode: proto.String(addreq.Code),
			InviteExpiration: proto.Int64(addreq.Expiration.UnixMilli()),
			GroupName: proto.String(grp.GroupName.Name),
			Caption: proto.String(invmsg),
		},
	}
	ts, err := cli.SendMessage(context.Background(), JID, msg, whatsmeow.SendRequestExtra{ID:""})
	if err != nil {
